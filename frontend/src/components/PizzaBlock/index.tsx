...
const PizzaBlock: React.FC<IPizzaBlock> = ({id, title, price, imageUrl, sizes, types}) =>{
    ...
    return (
        <div className="pizza-block-wrapper">
            ...
            <Link to={`/pizza/${id}`}>
                <img
                    className={`pizza-block__image ${styles.root}`} 
                    src={imageUrl}
                    alt="Pizza"
                />
            </Link>
            ...
            <div className="pizza-block__selector">
                <ul>
                    {types.map(type => <li key={type} onClick={() => setPizzaType(type)} className={pizzaType === type ? "active" : ""}> {type ? "Традиционное" : "Тонкое"}</li>)}
                </ul>
                <ul>
                    {sizes.map((size, i) => <li key={i} onClick={() => setPizzaSize(i)} className={pizzaSize === i ? "active" : ""}>{size} см</li>)}
                </ul>
            </div>
